...
public class UpdateBookServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		// cmd可能是DEL或是MOD，用來判斷前端傳來的是修改還是更新
		String cmd = request.getParameter("cmd");
		
		String bookIDStr = request.getParameter("bookID");
		//判斷前端需要哪種respose回復，ajax 非同步  normal 正常跳轉
		String async = request.getParameter("async");
		int bookID = Integer.parseInt(bookIDStr.trim());		
		if (cmd.equalsIgnoreCase("DEL")) {
	        sc.deleteBook(bookID); // 刪除購物車內的某項商品
	        RequestDispatcher rd = request.getRequestDispatcher("/ShowCartContentTest.jsp");
			...
			if("normal".equalsIgnoreCase(async)) {
	        RequestDispatcher rd = request.getRequestDispatcher("/ShowCartContentTest.jsp");
		    rd.forward(request, response);
		    return;
			}else {
