...
public class MemberService {
	...
	public boolean changePassword(String userName, String oldPassword, String newPassword) {
		...
		if (bean != null) {
			if (newPassword != null && newPassword.length() != 0) {
				memberDao.updatePassword(userName, newPassword);
				return true;
			}
		}
		return false;
	...
	public String createResetId(String email) {
		...
		//
		// // 3. 建立token()
		// ResetPasswordToken token = new ResetPasswordToken(startTime, endTime,
		// resetId, email);
		...
		// // 4. 建立url網址
		// String url =
		// Base64.getUrlEncoder().withoutPadding().encodeToString(tokentobyte);
